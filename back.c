...
void print_books(WINDOW * menu, int choice){
    ...
    wclear(menu);
    box(menu, 0,0);
    Livro *book = (Livro*) malloc(sizeof(Livro)), *temp_book;
        ...
        case 1:
            ...
            while(fread(&livro, sizeof(Livro), 1, file)){
                if(strstr(livro.nome_livro, sub_string) != NULL){
                    ...
                    k++;
                    temp_book = (Livro*) realloc(book, sizeof(Livro)*k);
                    book = temp_book;
                    achou = 1;
                }
            }
            ...
            for(i=0; i<k-1; i++){
                ...
                mvwprintw(menu, 6+(4*i),1, "Ano de publicação: %d e Quant. exemplares: %d", book[i].ano_publicacao, book[i].quantidade_exemplares);
            }
